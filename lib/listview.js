...
module.exports = function(options) {
  ...
  function get_content_children() {
    ...
    var useSearchBox = obj.searchBar && self.y > -obj.searchBar.height;
    var lastIndex = endItemIndex + 1;
    if(cachePool.length < lastIndex){
      lastIndex = cachePool.length;
    }
    
    if(useSearchBox === prevIsUsingSearchBox && prevStartIndex === startItemIndex && prevEndIndex === endItemIndex){
  ...
  function createCachePool(){
    console.log('creating cache pool');
    cachePool = [];
